package com.thinkware.houston.driver.aos.repository.remote.manager;
...
import com.thinkware.houston.driver.aos.repository.remote.SocketRequestCallback;
...
public class NetworkManager {
	...
	private HashMap<String, ResponsePacket> responsePackets;
	private HashMap<Integer, SocketRequestCallback> mCallbackListeners;
    ...
    // constructor
    //---------------------------------------------------------------------------------
	private NetworkManager(MainApplication application) {
		networkListeners = new ArrayList<>();
		mCallbackListeners = new HashMap<>();
		responsePackets = new HashMap<>();
		mConfiguration = application.getRepository().getConfig();
	}


    public static NetworkManager getInstance(MainApplication mainApplication) {
    	if (instance == null) {
    		instance = new NetworkManager(mainApplication);
	    }
	    return instance;
    }
	...
	public void addCallbackListener(int key, SocketRequestCallback callback) {
		mCallbackListeners.put(key, callback);
	}
...
//     * @param context
     ...
     */
    public synchronized void request(RequestPacket packet) {
...
/*
        ...
        LogHelper.e("packet : " +  messageType);
        LogHelper.e("packet : " +  mCallbackListeners.get(messageType));

	    Iterator<Map.Entry<Integer, SocketRequestCallback> >
			    iterator = mCallbackListeners.entrySet().iterator();
	    while (iterator.hasNext()) {
		    Map.Entry<Integer, SocketRequestCallback> entry = iterator.next();
		    LogHelper.e("key : " + entry.getKey());
	    }
*/
